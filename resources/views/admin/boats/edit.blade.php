    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.boats.update", [$boat->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $boat->description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('addresses') ? 'is-invalid' : '' }}" name="addresses[]" id="addresses" multiple>
                    @foreach($addresses as $id => $address)
                        <option value="{{ $id }}" {{ (in_array($id, old('addresses', [])) || $boat->addresses->contains($id)) ? 'selected' : '' }}>{{ $address }}</option>
                    @endforeach
                </select>
