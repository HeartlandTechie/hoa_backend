    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.addresses.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lot_number') ? 'is-invalid' : '' }}" type="text" name="lot_number" id="lot_number" value="{{ old('lot_number', '') }}">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('rental_flag') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="rental_flag" id="rental_flag" value="1" {{ old('rental_flag', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rental_owner') ? 'is-invalid' : '' }}" name="rental_owner_id" id="rental_owner_id">
                    @foreach($rental_owners as $id => $entry)
                        <option value="{{ $id }}" {{ old('rental_owner_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
