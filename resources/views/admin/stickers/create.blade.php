...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.sticker.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.stickers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="year">{{ trans('cruds.sticker.fields.year') }}</label>
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="text" name="year" id="year" value="{{ old('year', '2022') }}">
                @if($errors->has('year'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sticker_number') ? 'is-invalid' : '' }}" type="number" name="sticker_number" id="sticker_number" value="{{ old('sticker_number', '') }}" step="1">
